...
constellation.cpp // 백준 4386(별자리 만들기) 문제
city.cpp // 백준 1647(도시 분활 계획) 
```
**간단 내용**          
**DFS**             
- 깊이 우선 탐색           
...
-----
### 0317~0320           
코로나 격리 해제가 얼마 남지 않았다 넘 힘들었다 후'ㅁ'      
**생성파일**
```
network.cpp // 백준 1922(네트워크 연결) 문제
spanningTree.cpp // 백준 1197(최소 스패닝 트리) 문제
shortcut.cpp // 백준 1446(지름길) 문제
shortest.cpp // 백준 1753(최단 경로) 문제
```
**간단내용**              
- **다익스트라 알고리즘**
 - 최소 비용을 구할 때 유용한 알고리즘으로 주어진 두 노드 사이의 최소 비용 경로를 찾을 때 사용된다.
 - 간선의 가중치에 음수가 있다면 다익스트라 알고리즘을 적용할 수 없다.
 - 우선 순위 큐를 이용해 사용할 수 있다.
-----
### 0321~0326
**생성파일**
```
parcel.cpp // 백준 1719(택배) 문제
road.cpp // 백준 2307(도로 검문) 문제
timemachine.cpp // 백준 11657(타임머신) 문제 -> 실패(출력이 너무 많다고 뜬다 왤까?)
hole.cpp // 백준 1865(웜홀) 문제
```
**간단내용**
- 다익스트라 알고리즘과 벨만포드 알고리즘을 이용하여 문제를 풀었다.
- 벨만포드는 아직 익숙해지지 않아서 더 공부해봐야 할 것 같다.
- 중간고사 기간이라 알고리즘 스터디를 잠시 중단하도록 했다.
